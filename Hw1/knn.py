...
def get_nearest_neighbors(example_set, query, k):
    ...
    idx_of_nearest = np.argsort(dist)[:k]
...
def knn_classify_point(examples_X, examples_y, query, k):
    ...
    for i in neighbor_indices:
        nearest_neighbors_label.append(examples_y[i][0])
    predicted_label = max(set(nearest_neighbors_label), key=nearest_neighbors_label.count)
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
    ...
    sub_arr = np.split(train_X, num_folds)
    sub_arr_labels = np.split(train_y, num_folds)


    for i in range(num_folds):
        train_folded_X = np.vstack([fold for j, fold in enumerate(sub_arr) if j != i])
        train_folded_y = np.vstack([fold for j, fold in enumerate(sub_arr_labels) if j != i])

        #run knn for validation fold
        predicted_label = predict(train_folded_X, train_folded_y, sub_arr[i].tolist(), k)

        #compute accuracy for the fold
        accuracy = compute_accuracy(sub_arr_labels[i].tolist(), predicted_label)
