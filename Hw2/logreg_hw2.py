...
def main():
  ...
  pred_test_y = X_test_bias@w>=0 #stolen fromm line 25
  #raise Exception('Student error: You haven\'t implemented the code in main() to make test predictions.')
  
  #stolen from homework 1 (control c control v)
  test_out = np.concatenate((np.expand_dims(np.array(range(len(pred_test_y)),dtype=int), axis=1), pred_test_y), axis=1)
...
def logistic(z):
  ...
  #applying what we learned in lecutre
    # For z > 0, uwe -log(1 + e^z)
  #positive_mask = z > 0
  #logit_z[positive_mask] = -np.log(1 + np.exp(-z[positive_mask]))

    # For z <= 0, use -log(1 + exp(z)) + z
  #negative_mask = z <= 0
  #logit_z[negative_mask] = -np.log(np.exp(z[negative_mask]) + 1) + z[negative_mask]
  for i, z_num in enumerate(z):
    if z_num >= 0:
      logit_z[i] = 1 / (1 + np.exp(-z_num))
    if z_num < 0:
      logit_z[i] = np.exp(z_num) / (1 + np.exp(z_num))
  return logit_z
...
def calculateNegativeLogLikelihood(X,y,w):
  ...
  nll = -np.sum(y * np.log(temp + 1e-7) + (1 - y) * np.log(1 - temp + 1e-7))
  return nll
...
def trainLogistic(X,y, max_iters=max_iters, step_size=step_size):
    ...
    for i in range(max_iters):
        ...
        w_grad = np.dot(X.T, logistic(np.dot(X, w)) - y)
        # .
